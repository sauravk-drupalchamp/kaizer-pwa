...
const ajax = async () => {
  if (!singleton) {
    const tokenURL = config.drupal_local_url + "/session/token";
    // const tokenURL = config.drupal_live_url + '/rest/session/token';
    try {
      ...
      singleton = axios.create({
        baseURL: config.drupal_local_url, // every request is relative to this URL
        // baseURL: config.drupal_live_url, // every request is relative to this URL
      ...
      console.info("Created new axios instance", singleton);
    } catch (error) {
      console.error(error);
    }
  }
